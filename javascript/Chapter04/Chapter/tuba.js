...
var messageHeadElement;
/* global variables referencing sidebar h2 and p elements */
messageHeadElement = document.getElementById("messageHead");
...
function verifyAcres() {
	...
	try {
		...
		validity = false;
		messageText = message;
		acresBox.value = ""; // remove errorneous entry from input box
	}
	finally {
...
function verifyMonths() {
   ...
   try {
	   if (!(monthsBox.value >= 1 && monthsBox.value <= 12)) {
		   throw "Please enter a number of number of months between 1 and 12";
	   }
   }
   catch (message) {
		validity = false;
